...
OTHERS
    : . {
        writeIntoLexLogFile(
          "Line# " + std::to_string(getLine())
          + ": Token <OTHERS> Lexeme " + getText()
        );
        
        errorFile << "Error: Unrecognized character '" << getText() << "' at line " << getLine() << std::endl;
        parserLogFile << "Error: Unrecognized character '" << getText() << "' at line " << getLine() << std::endl;
    }
    ;
